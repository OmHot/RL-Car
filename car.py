...
class Car:
    ...
    def _compute_radars(self, screen, surface):
        ...
        car_angles = np.array([radians(90 - self.angle - i) for i in [15,75,90,105,165]])##########################
                               #(60*(angle//2+1) if angle%2 ==1 else 45*angle) for angle in range(5)])
        #[radians(90 - self.angle - 45 * angle) for angle in range(5)])
    ...
    def translate_position(self):
        current_position, angle_degrees, distance = self.position,self.angle,self.velocity.x
        angle_radians = radians(angle_degrees)
        delta_x = distance * cos(angle_radians)
        delta_y = distance * sin(angle_radians)
        new_position = np.array(current_position) + np.array([delta_x, delta_y])
        return tuple(new_position)
    def npc_move(self):
         # Get the car's current position and angle
        # Define the distance to move the car (e.g., based on speed)

        # Calculate the new position
        new_position = self.translate_position()

        # Update the car's position
        self.position = new_position

        # Update other aspects of the car's state as needed (e.g., radars, collision checks, scoring)
