...
class Highway:
    ...
    def scale(self,scale_factor):
        center_x = np.mean(self.highway_curve[:, 0])
        center_y = np.mean(self.highway_curve[:, 1])

# Define the scale factor (e.g., 2 for doubling the size)

# Translate the curve to the origin
        translated_curve = self.highway_curve - np.array([center_x, center_y])

# Scale the translated curve
        scaled_curve = translated_curve * scale_factor
# Translate the scaled curve back to the original position (center)
        scaled_and_translated_curve = scaled_curve + np.array([center_x, center_y])
        return(scaled_and_translated_curve)
    def draw(self, screen):
        """Renders highway curve and its markup"""
        screen.fill(self.grass_color)
        for coord in self.highway_curve:
            pg.draw.circle(screen, self.markup_color, coord, 1.1 * self.width) #Road ends
        for coord in self.highway_curve:
        ...
        #for pts in self.highway_curve:
            #pg.draw.circle(screen,(255,0,0,255),pts,2)
